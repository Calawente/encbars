...
function funConvertWebsite( $options )
{
  ...
  while ( $url = $stmt->fetch( PDO::FETCH_ASSOC ) ) {
    ...
    $destination = str_replace( DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR, $output . DIRECTORY_SEPARATOR . $url['hostname'] . DIRECTORY_SEPARATOR . $pathinfo['dirname'] . $filename ); // fixed
                    ...
                    <div class="tab-pane fade pb-0  <?=( $subSection == 'loader' ) ? 'show active' : ''?>" id="settings_loader" role="tabpanel" aria-labelledby="settings_loader_tab">
                      ...
                      <form action="" method="post" id="form_loader_settings" class="needs-validation" novalidate>
                        <div class="form-group">
                          ...
                          <select class="form-control" name="settings[ARCHI\VARIX_LOADER_MODE]" id="loader_settings_mode" required>
                            <?php
                            $loaderModes = [
